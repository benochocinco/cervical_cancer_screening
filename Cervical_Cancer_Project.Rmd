...
We begin with looking at the Biopsy variable, as it is our outcome variable. Of the 726 observations remaining after cleaning our data, only 50 of them (6.9%) have a positive biopsy outcome. We will address this imbalance in the predictive modeling portion of our presentation. 

Considering our goal of building a highly predictive model for identifying cases of cervical cancer, we believe that exploring our 30 predictors is a crucial aspect of identifying relationsips that may exist between them and Biopsy outcome variable. The imbalance of the Biopsy variable beggs the question of whether or not specific values of certain predictors can be tied to the positive Biopsy cases. None of our predictors are continuous, so we can divide our discrete variables into two categories for further exploration: integer and boolean.

## Integer Data

We begin with simple scatter plots and stacked bar graphs for our integer predictors. Plotting each integer predictor versus Biopsy outcome with the scatter plots allows us to compare the integer values of the points with a positive cancer biopsy to those with a negative one. The stacked bar graphs allow us to see relative frequencies of positve and negative biopsies for each value of an integer predictor. 

```{r, include= TRUE}
#List of all 9 non-boolean predictors
non_booleans <- c("Age", "Number.of.sexual.partners", "First.sexual.intercourse", "Smokes..years.", "Smokes..packs.year.", "Hormonal.Contraceptives..years.", "IUD..years.", "STDs..number.", "STDs..Number.of.diagnosis") 

#Set up a 3x3 plotting space
par(mfrow = c(2, 5))

#Plot predictors vs. biopsy result
for (non_boolean in non_booleans){
  y_value2 <- subset(cancer_df, select = non_boolean)
  y_value2 <- (unlist(y_value2, use.names=FALSE))
  plot(x = cancer_df$Biopsy, y = y_value2, type = "p", xlab = "Biopsy Result", ylab = non_boolean)
  
  temporary_df <- subset(cancer_df, select = c(non_boolean, "Biopsy"))
  unique_values <- c()
  positives <- c()
  negatives <- c()
  for (value in unique(y_value2)){
    if (value%%1==0){
      unique_values <- c(unique_values, value)
      has_this_value <- subset(temporary_df, temporary_df[,1] == value)
      negatives <- c(negatives, sum(!has_this_value$Biopsy))
      positives <- c(positives, sum(has_this_value$Biopsy))
    }
  }
  this_variable_df <- data.frame(unique_values, positives, negatives)
  tmp <- melt(this_variable_df, id.vars = "unique_values",
              measure.vars = c("positives", "negatives"))
  print(ggplot(tmp, aes(x = unique_values, y = value, fill = variable)) + 
  geom_bar(stat = "identity") +
  scale_fill_manual(values=c("#E69F00", "#56B4E9"),name="Biopsy",breaks=c("positives", "negatives"), labels=c("Positive Result","Negative Result")) +
  xlab(non_boolean) +
  ylab("Count")) 
...
```{r, include= TRUE}
#List of all 21 boolean predictors
booleans <- c("Smokes", "Hormonal.Contraceptives", "IUD", "STDs", "STDs.condylomatosis", "STDs.cervical.condylomatosis", "STDs.vaginal.condylomatosis", "STDs.vulvo.perineal.condylomatosis", "STDs.syphilis", "STDs.pelvic.inflammatory.disease", "STDs.genital.herpes", "STDs.molluscum.contagiosum", "STDs.AIDS", "STDs.HIV", "STDs.Hepatitis.B", "STDs.HPV", "Dx.Cancer", "Dx.CIN", "Dx.HPV", "Dx")

#Data frame with only positive biopsies
positive_biopsies <- subset(cancer_df, Biopsy == 1)

#Creating data frame for chart
boolean_df <- data.frame(matrix(nrow = length(booleans), ncol = 3))
colnames(boolean_df) <- c("Predictor","Count Positive", "Percent Positive")
boolean_df$Predictor <- booleans

#Getting counts for chart
count_vector <- c()
for (boolean in booleans){
  column <- subset(positive_biopsies, select = boolean)
  count <- sum(column == 1)
  count_vector <- c(count_vector, count)
}

#Adding counts and percents to table
#percent_vector <- (count_vector/50) * 100
percent_vector <- c("20%", "66%", "18%", "22%", "14%",  "0%",  "0%", "14%",  "0%",  "0%",  "2%",  "0%",  "0%",  "8%",  "0%",  "0%", "12%",  "4%", "12%", "12%")
boolean_df$`Count Positive` <- count_vector
boolean_df$`Percent Positive` <- percent_vector

kable(boolean_df, format = "markdown", align = "l")
```
